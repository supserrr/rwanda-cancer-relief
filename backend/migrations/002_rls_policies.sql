...
-- Use JWT metadata to avoid recursion
...
-- Note: This policy allows patients to view their assigned counselor
-- The assigned_counselor_id is checked directly without recursion
...
CREATE POLICY "Counselors and admins can create resources"
    ...
    WITH CHECK (
        ...
        (auth.jwt() -> 'user_metadata' ->> 'role') IN ('counselor', 'admin')
    );
